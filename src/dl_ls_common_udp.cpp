...
CdlLsCommonUdp::CdlLsCommonUdp(const std::string &hostname, const std::string &port, int &timelimit, CParserBase *parser, ros::NodeHandle &nh, ros::NodeHandle &pnh) :
    CdlLsCommon(parser, nh, pnh),
    mHostName(hostname),
    mPort(port),
...
CdlLsCommonUdp::~CdlLsCommonUdp()
{
    StopScanner();
    CloseDevice();
}

int CdlLsCommonUdp::InitDevice()
{
    int opt = 1;
...
int CdlLsCommonUdp::CloseDevice()
{
    if(mSocket != -1)
    {
        close(mSocket);
...
int CdlLsCommonUdp::SendUdpData2Device(char *buf, int length)
{
    int n = -1;
    if(mSocket > 0)
    {
...
int CdlLsCommonUdp::SendDeviceReq(const char *req, std::vector<unsigned char> *resp)
{
    if(mSocket == -1)
    {
        ROS_ERROR("SendDeviceReq: Socket NOT open");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "dl_LS - SendDeviceReq: socket NOT open!");
    ...
    {
        ROS_ERROR("dlite error for req command");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "dl_LS - SendDeviceReq: dlite command failed!");
...
int CdlLsCommonUdp::GetDataGram(unsigned char *receiveBuffer, int bufferSize, int *length)
{
    int len;
    struct timeval tv;
    socklen_t addrlen;
    ...
    if(mSocket == -1)
    {
        ...
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "dl_LS - GetDataGram: socket NOT open!");
    ...
    if(1)//(!stream_stopped_)
    {
        ...
        {
            std::string errorStr =  "dl_LS - GetDataGram: No full response for read after " + mTimeLimit;
            errorStr = errorStr + "S";
