...
namespace dl_ls_udp
{
CdlLsCommon::CdlLsCommon(CParserBase *parser, ros::NodeHandle &nh, ros::NodeHandle &pnh) :
    mDiagPublisher(NULL),
    ...
    mPrivNodeHandler(pnh)
{
    ...
    dynamic_reconfigure::Server<dl_ls_udp::dlLsConfig>::CallbackType f;
    f = boost::bind(&dl_ls_udp::CdlLsCommon::UpdateConfig, this, _1, _2);
...
bool CdlLsCommon::RebootDevice()
{
    ...
    if(0 != result)
    {
        ROS_ERROR("dl_LS - Error setting access mode");
        mDiagUpdater.broadcast(diagnostic_msgs::DisgnosticStatus::ERROR,
                               "dl_LS - Error setting access mode");
    ...
    {
        ROS_ERROR_STREAM("dl_LS - Error setting access mode, unexpected response : " << strAccessResp);
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "dl - Error setting access mode.");
    ...
    if(0 != result)
    {
        ROS_ERROR("dl_LS - Error rebooting device");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "dl_LS - Error rebooting device");
    ...
    {
        ROS_ERROR_STREAM("dl_LS - Error setting access mode, unexpected response : " << strRebootResp);
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "dl_LS - Error rebooting device");
    ...
    ROS_INFO("dl_LS - Rebooted scanner");
#endif
    return true;
}

CdlLsCommon::~CdlLsCommon()
{
    delete mDiagPublisher;
    ROS_INFO("dl_ls_udp drvier exiting.\n");
}

int CdlLsCommon::Init()
{
...
int CdlLsCommon::InitScanner()
{
    ...
    if(0 != result)
    {
        ROS_ERROR("dl_LS - Error reading variable 'DeviceIdent'.");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "dl_LS - Error reading variable 'DeviceIdent'.");
    }
    ...
    if(0 != result)
    {
        ROS_ERROR("dl_LS - Error reading variable 'SerialNumber'.");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "dl_LS - Error reading variable 'SerialNumber'.");
    }
    ...
    {
        ROS_ERROR("dl_LS - Error Unsuppored identify %s", strIdentify);
        return ExitFatal;
    }
    ...
    if(0 != result)
    {
        ROS_ERROR("dl_LS - Error reading variable 'FirmwareVersion'.");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "dl_LS - Error reading variable 'FirmwareVersion'.");
    }
    ...
    if(0 != result)
    {
        ROS_ERROR("dl_LS - Error reading variable 'devicestate'.");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "dl_LS - Error reading variable 'devicestate'.");
    }
    ...
    if(0 != result)
    {
        ROS_ERROR("dl_LS - Error when starting streaming 'LMDscandata'.");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "dl_LS - Error when starting streaming 'LMDscandata'.");
...
std::string CdlLsCommon::StringResp(const std::vector<unsigned char> &resp)
{
    std::string strResp;
...
bool CdlLsCommon::IsCompatibleDevice(const std::string strIdentify) const
{
    // TODO: Always return true
    return true;
}

int CdlLsCommon::LoopOnce()
{
    ...
    if(0 != result)
    {
        ROS_ERROR("dl_LS - Read Error when getting datagram: %d", result);
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
                               "dl_LS - Read Error when getting datagram.");

        return ExitError;
    }
    else
    {
        ROS_DEBUG("dl_LS - Received data gram. Data Length %d", dataLength);
        if(!mConnectFlag)
        {
            mConnectFlag = 1;
            ROS_INFO("dl_LS - Successfully connected !!");
        }
    ...
    if(start == NULL)
    {
        ROS_ERROR("dl_LS - Invalide data! Header NOT found!");
    }
...
void CdlLsCommon::CheckAngleRange(dl_ls_udp::dlLsConfig &config)
{
...
void CdlLsCommon::UpdateConfig(dl_ls_udp::dlLsConfig &newConfig, uint32_t level)
{
    CheckAngleRange(newConfig);
    mConfig = newConfig;
}

void CdlLsCommon::DumpLaserMessage(sensor_msgs::LaserScan &msg)
{
...
void CdlLsCommon::ClearConnectFlag(void)
{
    mConnectFlag = 0;
}
