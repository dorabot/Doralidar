#ifndef dl_LS_COMMON_H_
#define dl_LS_COMMON_H_
...
public:
    CdlLsCommon(CParserBase *parser, ros::NodeHandle &nh, ros::NodeHandle &pnh);
    virtual ~CdlLsCommon();
    virtual int  Init();
    int          LoopOnce();
    void         CheckAngleRange(dl_ls_udp::dlLsConfig &config);
    void         UpdateConfig(dl_ls_udp::dlLsConfig &newConfig, uint32_t level = 0);
...
protected:
     ...
     * \param [out] length the actual amount of data dlitten.
     * */
...
private:
    ...
    // Dynamic Reconfigure
    dlLsConfig  mConfig;
    dynamic_reconfigure::Server<dl_ls_udp::dlLsConfig> mDynaReconfigServer;
};
} // dl_ls_udp

#endif // dl_LS_COMMON_H_
